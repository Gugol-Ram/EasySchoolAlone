...
import { Button, Spinner } from "react-bootstrap"; // Importa Spinner
  ...
  useEffect(() => {
    if (filteredParentId) {
      dispatch(getParent(filteredParentId))
        .then(() => setLoading(false)) // Termina la carga cuando los datos se obtienen
        .catch(() => setLoading(false)); // Asegúrate de manejar errores y terminar la carga
      sessionStorage.setItem("parentId", filteredParentId);
    } else {
      setLoading(false); // Termina la carga si no hay parentId
    }
          ...
          {parentId.parentDetails && parentId.parentDetails.length > 0 ? (
            ...
            <div className={style.notRegistered}>
              ...
              <p className={style.notRegisteredMsg}>
                ...
                manera más completa con la página, como proceder a la
                inscripción del Estudiante y/o dejarnos tu opinión.
              </p>
